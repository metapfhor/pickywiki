...
d3.layout.stripPacker = function(events,start,end){
	...
	var offset;
	var packer = {};
	function d3_eventstripNode(x1,x2,parent) {
		var node ={};
		node.left=null;
		node.right=null;
		node.point=null;
		...
		node.inlay=function(event,offset){
			...
			event.propagate("y",offset);
			event.stripNode=node.left.right;
	...
	packer.strips=[];
	packer.add=function(event,dest){
		function insert(event,y){
			if(dest!=undefined){
				

				for(var i=0;i<=y;i++){
					packer.strips[i]=d3_eventStrip(t1,t2);
				}
				packer.strips[y].insert(event,y);
			}else{
