...
function updateNodes(callback) {
        smwAsk('[[Category:StickyWiki/2012]] OR [[Category:StickyWiki]]&po=Endangers|Helps|Links|Causes|Requires', function(data) {
                $('#canvas').html('<img src="Loader.gif" alt="spinner" /> Loading map …');

                $.each(data.ask.results.items, function(key, val) {
                        c = {};
			c.keyid = key;
			links = [];
                        c.pagename = val['title'];
			$.each(['endangers', 'helps', 'links', 'causes', 'requires'], function(key, cat) {
				if (val.properties[cat]) {
					links.push(val.properties[cat]);
				}
			});
			c.links = links;
			c.type = val.properties.type;
