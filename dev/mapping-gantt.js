...
data.latest=(data.latest+data.earliest)/2;

focusViz=d3.select("#graph").append("svg:svg").attr("width",w).attr("height",h).style("display","inline");
d3.select("#graph").append("br");
contextViz=d3.select("#graph").append("svg:svg").attr("width",w).attr("height",h/3).style("display","inline");

cont=d3.select("#contents").append("div").attr("width",200).attr("height",h).style("display","inline");

var foc = ganttFocus(focusViz,data);
var context = ganttContext(contextViz,foc);

filter=function(elm){
	foc.filter(elm.value.toLowerCase())
}
