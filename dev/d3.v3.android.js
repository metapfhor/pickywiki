(function() {
  ...
  //added bound zooming for performance gains on mobile bound zooming

  d3.behavior.boundZoom = function() {
   // var translate = [ 0, 0 ], translate0, scale = 1, scale0, scaleExtent = d3_behavior_zoomInfinity, event = d3_eventDispatch(zoom, "zoom"), x0, x1, y0, y1,touchtime;
 	var translate = [ 0, 0 ], translate0, scale = 1, scale0, scaleExtent = d3_behavior_zoomInfinity, event = d3_eventDispatch(zoom, "zoom"), x0, x1, y0, y1,rng, startloc, touchtime, origRngX, widthX, origRngY, widthY;
    function zoom() {
    ...
    zoom.x = function(z) {
      if (!arguments.length) return x1;
      widthX = z.range();
      origRngX = [widthX[0],widthX[1]];
      widthX = widthX[1]-widthX[0];
      x1 = z;
      x0 = z.copy();
      translate = [ 0, 0 ];
      scale = 1;
    ...
    zoom.y = function(z) {
      if (!arguments.length) return y1;
      widthY =  z.range();
      origRngY = [widthY[0],widthY[1]];
      widthY = widthY[1]-widthY[0];
      y1 = z;
      y0 = z.copy();
      orig
      translate = [ 0, 0 ];
    ...
    function rescale() {
      if (x1){
        //rng = x0.range().map(function(x){return x;});
        rng = [origRngX[0],origRngX[1]];
       // translate[0]=Math.min(0,translate[0]);
       // translate[0]=Math.max(translate[0],widthX-widthX*scale)
       // rng[0]=(rng[0]-translate[0])/scale;
      //  rng[1]=(rng[1]-translate[0])/scale;
       // x1.domain([x0.invert(rng[0]),x0.invert(rng[1])]);
        x1.domain(x0.range().map(function(x) { return (x - translate[0]) / scale;}).map(x0.invert));
      } 
      if (y1){
        //rng = y0.range().map(function(x){return x;});
        rng = [origRngY[0],origRngY[1]];
        translate[1]=Math.min(0,translate[1]);
        translate[1]=Math.max(translate[1],widthY-widthY*scale)
        rng[0]=(rng[0]-translate[1])/scale;
        rng[1]=(rng[1]-translate[1])/scale;
        y1.domain([y0.invert(rng[0]),y0.invert(rng[1])]);
      } 
    }
  ...
  d3.behavior.zoom = function() {
   // var translate = [ 0, 0 ], translate0, scale = 1, scale0, scaleExtent = d3_behavior_zoomInfinity, event = d3_eventDispatch(zoom, "zoom"), x0, x1, y0, y1,touchtime;
    ...
    function zoom() {
      this.on("mousedown.zoom", mousedown).on("mousewheel.zoom", mousewheel).on("mousemove.zoom", mousemove).on("DOMMouseScroll.zoom", mousewheel)/*no double click zoom.on("dblclick.zoom", dblclick)*/.on("touchstart.zoom", touchstart).on("touchmove.zoom", touchmove).on("touchend.zoom", touchstart);
    }
    zoom.translate = function(x) {
      if (!arguments.length) return translate;
      translate = x.map(Number);
