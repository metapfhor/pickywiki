...
function drawQuads(){
	quads.selectAll("rect").remove();
	q = d3.geom.quadtree(mds.nodes());
	q.visit(drawQuad(picky.nodes[Math.floor(Math.random()*picky.nodes.length)]));
...
function drawQuad(node){ 
	...
	return function(quad,x1,y1,x2,y2){
		var rect=quads.append("svg:rect");
		if(!quad.leaf){
			rect.attr("fill","red").attr("fill-opacity","0");
		}else{
			rect.attr("fill","blue").attr("fill-opacity",1/n);
		}
		rect.attr("stroke","black").attr("stroke-width","1").attr("stroke-opacity","0.5")
		.attr("x",x1).attr("y",y1).attr("width",x2-x1).attr("height",y2-y1);
		n++;
	};
}

inform = function (node){return function(quad,x,y,z,w){if(!quad.leaf)console.info(quad)}}
