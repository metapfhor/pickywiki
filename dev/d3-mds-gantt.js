...
d3.layout.mdsGantt = function() {
	...
	function dragmove(d) {
		...
		if(change){
			if(max>end){
				dx=max-end;
				mds.timedomain([time.invert(0),time.invert(size[0]+dx)]);
			}else{
				if(min<begin){
					dx=min-begin;
					mds.timedomain([time.invert(dx),time.invert(size[0])]);
				}
			}
    ...
    var mds = {}, event = d3.dispatch("start","tick","domain","scope","end"), size = [ 0, 0 ], alpha,drag,padding=[0,0], nodes = [], links = [], neighbors, distance,weight=[],nan=false,x,y,px,py,neighbors;
   ...
   function Dijkstra(src){
	...
	return dist;
      ...
      for (i = 0; i < n; ++i) {
		...
		if(o.startTime+o.duration>lastEnd)lastEnd=o.startTime+o.duration
        o.weight = 0;
		o.links = [];
      }
      for (i = 0; i < m; ++i) {
