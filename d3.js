d3 = function() {
  ...
  d3.behavior.drag = function() {
    ...
    d3.behavior.directionalZoom = function() {
    var translate = [ 0, 0 ], translate0, scale = [1,1], distance0, scale0, scaleExtent = [d3_behavior_zoomInfinity,d3_behavior_zoomInfinity], mousedown = "mousedown.zoom", mousemove = "mousemove.zoom", mouseup = "mouseup.zoom" ,event = d3_eventDispatch(zoom, "zoom"), x0, x1, y0, y1, touchtime, alt=false, altKey="Shift",thresh=4;
    function zoom() {
      this.on(mousedown, mousedowned).on(d3_behavior_zoomWheel + ".zoom", mousewheeled).on(mousemove, mousewheelreset).on("touchstart.zoom", touchstarted);
      d3.select(window).on("keydown.zoom",listenForAlt);
    }
    ...
    zoom.scale = function(x) {
      if (!arguments.length) return [scaleX,scaleY];
      if(!x.length){
        scale = [Math.abs(x),Math.abs(x)];
      }else{
        scale = [Math.abs(x[0]),Math.abs(x[1])];
      }
      rescale();
      return zoom;
    };
    zoom.scaleExtent = function(x) {
      ...
      scaleExtent = x == null ? [d3_behavior_zoomInfinity,d3_behavior_zoomInfinity] : [x[0].map(Number),x[1].map(Number)];
      console.info(scale);
      return zoom;
    };
    zoom.x = function(z) {
    ...
    zoom.altKey = function(z){
      if(!arguments.length) return altKey;
    ...
    zoom.aspectThreshold = function(z){
      if(!argumetns.length) return thresh;
      thresh = z;
      return zoom;
    }
    function location(p) {
      return [ (p[0] - translate[0]) / scale[0], (p[1] - translate[1]) / scale[1]];
    }
    function point(l) {
      return [ l[0] * scale[0] + translate[0], l[1] * scale[1] + translate[1] ];
    }
    function scaleTo(s) {
      scale = [Math.max(scaleExtent[0][0], Math.min(scaleExtent[0][1], s[0])),Math.max(scaleExtent[1][0], Math.min(scaleExtent[1][1], s[1]))];
    }
    ...
    function rescale() {
      if (x1) x1.domain(x0.range().map(function(x) {
        return (x - translate[0]) / scale[0];
      }).map(x0.invert));
      if (y1) y1.domain(y0.range().map(function(y) {
        return (y - translate[1]) / scale[1];
      }).map(y0.invert));
    }
    ...
    function touchstarted() {
     ...
     if (touches.length > 1) {
        ...
        distance0 = [Math.abs(dx),Math.abs(dy)];
      }
      function moved() {
        ...
        if (p1 = touches[1]) {
          ...
          if (scale1 == null) {
            var distance1 = [Math.abs(p1[0] - p0[0]) ,Math.abs(p1[1] - p0[1])];
            if(distance0){
              if(distance0[0]/distance0[1]>thresh){
                scale1 = [distance1[0] / distance0[0], 1];
              }else if(distance0[1]/distance0[0]>thresh){
                scale1 = [1, distance1[1] / distance0[1]];
              }else{
                scale1 = [distance1[0] / distance0[0],distance1[1] / distance0[1]];
              }            
              
            }
            //scale1 = distance0 && [distance1[0] / distance0[0],distance1[1] / distance0[1]];
          }
          p0 = [ (p0[0] + p1[0]) / 2, (p0[1] + p1[1]) / 2 ];
          l0 = [ (l0[0] + l1[0]) / 2, (l0[1] + l1[1]) / 2 ];
          scaleTo([scale1[0] * scale0[0],scale1[1] * scale0[1]]);
        }
        touchtime = null;
        translateTo(p0, l0);
        dispatch(event_);
    ...
    function mousewheeled() {
      ...
      if(!alt){
        scaleTo([Math.pow(2, d3_behavior_zoomDelta() * .002) * scale[0],scale[1]]);
      }else{
        scaleTo([scale[0],Math.pow(2, d3_behavior_zoomDelta() * .002) * scale[1]]);
      }
    ...
    function listenForAlt(){
      function altUp(e){
        if(e.keyIdentifier==altKey){
          alt=false;
          this.removeEventListener("keyup",altUp);
        }
      }
      if(d3.event.keyIdentifier==altKey){
        alt=true;
        this.addEventListener("keyup",altUp)
      }
    }
    console.info(scale)
