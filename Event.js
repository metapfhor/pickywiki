...
root.gesture = function(conf) {
	...
	conf.onPointerDown = function (event) {
		...
		if (conf.fingers === conf.minFingers && fingers !== conf.fingers) {
			...
			//Laurent Modification: directional discrimination && target bound scales for zooming history
			
			self.scale = (typeof(conf.target.scale)!="undefined"?conf.target.scale:1);
			self.scaleX = (typeof(conf.target.scaleX)!="undefined"?conf.target.scaleX:1);
			self.scaleY = (typeof(conf.target.scaleY)!="undefined"?conf.target.scaleY:1);
			//end Laurent
			self.rotation = 0;
			self.state = "start";
	...
	conf.onPointerMove = function (event, state) {
		...
		var touches = [];
		var scale = 0;
		//Laurent Modification: directional discrimination
		var scaleX = 0;
		var scaleY = 0;
		//end Laurent
		var rotation = 0;
		...
		for (var sid in points) {
			...
			if (!start.distance) {
				...
				//Laurent Modification: directional discrimination
				start.distanceX=Math.abs(dx)||1;
				start.distanceY=Math.abs(dy)||1;
				//end Laurent
			}

			// Calculate scale.
			//Laurent Modification: zero scale is getting really annyoing
			var dx = (touch.move.x - centroidx)||1;
			var dy = (touch.move.y - centroidy)||1;
			var distance = Math.sqrt(dx * dx + dy * dy);
			scale += distance / start.distance;
			//Laurent Modification: directional discrimination
			scaleX += Math.abs(dx)/start.distanceX;
			scaleY += Math.abs(dy)/start.distanceY;
			//end Laurent
			// Calculate rotation.
		...
		//Laurent Modification: directional discrimination
		self.scaleX = scaleX / conf.fingers;
		self.scaleY = scaleY /conf.fingers;
		//write scales to target if already present
		if(typeof(conf.target.scale)!="undefined")conf.target.scale=self.scale;
		if(typeof(conf.target.scaleX)!="undefined")conf.target.scaleX=self.scaleX;
		if(typeof(conf.target.scaleY)!="undefined")conf.target.scaleY=self.scaleY;
		//end Laurent
		self.rotation = rotation / conf.fingers;
		self.state = "change";//hacky as hell
		conf.listener(event, self);
	};
	conf.onPointerUp = function(event) {
		// Remove tracking for touch.
